...
# MODULE D: BULK DATA SCAN (Mass Audit)
    ...
    test_id = st.text_input("Enter Node ID", value="ANKIT_24SPCD002")
    ...
    if st.button("Map Hidden Links"):
        with st.spinner("AI is tracing multi-hop connections..."):
            ...
            <div style='background: rgba(255,255,255,0.05); padding: 20px; border-radius: 15px;'>
                ...
                <p> &nbsp; ↳ <span style='color:#00ffcc;'>Edge: Verified Device</span></p>
                <p> &nbsp; ↳ <span style='color:#ff4b4b;'>Edge: Fraudulent Cluster Link</span> ---> [Node_ID: BLACKLIST_04]</p>
            </div>
            """, unsafe_allow_html=True)
            st.error("🚨 ALERT: Deep-Link Fraud Detected via GNN!")

# 7. TECHNICAL ARCHITECTURE (Explainable AI Module)
st.divider()
with st.expander("🛠️ TECHNICAL EXPLAINER (For Viva Discussion)"):
