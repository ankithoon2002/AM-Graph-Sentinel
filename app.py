...
if not st.session_state.logged_in:
    st.markdown(
        "<h1 style='text-align: center; color: #58a6ff;'>🛡️ AM SENTINEL SECURE ACCESS</h1>",
        unsafe_allow_html=True
    )
...
# --- 6. HEADER ---
st.markdown(
    "<h1 style='text-align: center; color: #58a6ff;'>🛡️ AM UNIVERSAL FRAUD SENTINEL</h1>",
    unsafe_allow_html=True
)
...
if show_feed:
    st.info(
        f"📡 SCANNING: {random.choice(['Node-X22', 'SBI Gateway', 'Paytm-API', 'HDFC Server'])}"
    )
...
if st.session_state.page == 'home':
    ...
    with c1:
        if st.button("📱 UPI Wallet"):
            navigate('upi')

    with c2:
        if st.button("📑 Check Audit"):
            navigate('check')

    with c3:
        if st.button("🕸️ GNN Network"):
            navigate('graph')
    ...
    with c4:
        if st.button("🏥 Medical Insurance"):
            navigate('ins')

    with c5:
        if st.button("⚖️ Tax/GST Forensics"):
            navigate('tax')

    with c6:
        if st.button("🏢 Loan Risk"):
            navigate('loan')
    ...
    if show_map:
        ...
        map_df = pd.DataFrame({
            'lat': [28.6, 19.0, 13.0, 22.5],
            'lon': [77.2, 72.8, 80.2, 88.3],
            'risk': [20, 95, 40, 70]
        })
...
# GNN PAGE
    ...
    with cg:
        nodes = [
            ...
            Node(id="F", label="FRAUD", color="#d12d3d", size=35)
        ]

        edges = [
            Edge(source="B", target="W"),
            Edge(source="W", target="F"),
        ...
        agraph(
            nodes=nodes,
            edges=edges,
            config=Config(width=700, height=450, directed=True, physics=True)
        )
